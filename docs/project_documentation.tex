...
# Step 3: Start the simulation container
...
The simulation world (\texttt{reach\_avoid\_arena.sdf}) defines:
\begin{itemize}[noitemsep]
    \item $8 \times 8 \times 4$\,m room with four walls
    \item Red obstacle box at $x \in [3,4]$, $y \in [2,6]$\,m
    \item Green transparent target region at $x \in [6,8]$, $y \in [3,5]$\,m
    \item DART physics at 1\,kHz, real-time factor 1.0
    \item Two PX4 x500 quadrotor models (defender and attacker)
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \item Gazebo model names (\texttt{x500\_defender}, \texttt{x500\_attacker})
