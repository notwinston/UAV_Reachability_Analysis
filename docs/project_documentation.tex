...
\begin{itemize}
    ...
    \item \textbf{Online control:} A real-time ROS\,2 controller reads drone states, interpolates precomputed value functions, extracts optimal controls via gradient computation, and publishes velocity commands at 50\,Hz. When outside the proven winning region, a PID pursuit controller drives the defender directly toward the attacker.
...
The defender interception logic has been validated using a standalone kinematic simulation (\texttt{tests/integration/test\_kinematic\_game.py}) that replicates the full ROS\,2 kinematic game without requiring ROS\,2 or Gazebo. The simulation uses:
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Defender}: double integrator dynamics ($\dot{v} = K(u - v)$, gains from \texttt{game\_params.yaml})
    \item \textbf{Attacker}: single integrator with scripted waypoint following at 1.6\,m/s
    \item \textbf{Capture condition}: horizontal distance $\leq 3$\,m and vertical distance $\leq 1$\,m
\end{itemize}

\begin{table}[h]
\centering
\caption{End-to-end simulation results (all scenarios captured successfully).}
\begin{tabular}{lccl}
\toprule
...
In all scenarios, the defender uses \textbf{PID pursuit} mode (outside the winning region on the dev grid) and successfully intercepts the attacker. The controller transitions to \textbf{tracking} mode as the vertical relative state enters the invariant set $\mathcal{B}_z$.
