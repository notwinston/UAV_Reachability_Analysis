...
\begin{enumerate}
    \item \textbf{Attacker} spawns at $(5, 20, 3)$ and follows 6 waypoints:
    \begin{itemize}[noitemsep]
        \item $(5, 12.5, 10) \to (12, 12.5, 10)$: approach obstacle
        \item $(12, 3, 10) \to (25, 3, 10)$: route around obstacle below $y=5$
        \item $(25, 12.5, 10) \to (41.5, 12.5, 10)$: re-center and reach target
    \end{itemize}
    Speed: 1.6\,m/s ($= 2.0 \times 0.8$ speed fraction). Reaches target in $\sim$30--40\,s of flight.
    ...
    \begin{itemize}[noitemsep]
        ...
        \item Computes optimal velocity via value function gradients (Algorithm 1 + 2)
        \item Velocity clamped to 1.5\,m/s horizontal, 1.0\,m/s vertical for PX4 stability
    \end{itemize}
...
\begin{itemize}[noitemsep]
    \item \texttt{scripted} (default) --- follows predefined waypoints toward target
    \item \texttt{keyboard} --- manual control via \texttt{teleop\_twist\_keyboard}
    \item \texttt{optimal} --- uses value function gradient for optimal escape (stub)
    \item \texttt{switchable} --- switch between modes at runtime via ROS\,2 service
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \item Clamps velocities to 1.5\,m/s horizontal, 1.0\,m/s vertical for flight stability
...
\begin{description}[style=nextline, leftmargin=0.5cm]
    ...
    The reach-track controller outputs maximum velocity ($U_D = 6$\,m/s), which causes motor saturation in PX4's x500 model. The PX4 adapter clamps velocities (1.5\,m/s horizontal, 1.0\,m/s vertical). If the defender still flies erratically, reduce these clamp values or use the kinematic simulation mode.
