...
1. V_h_T: Legacy 4D relative maximum distance diagnostic
2. phi_A_reach: Attacker reaching value function (2D)
3. V_h_T_6d: Obstacle-aware maximum distance value function
4. B_h: Invariant capture set derived from obstacle-aware V_h_T_6d
5. Phi_h: Horizontal reach-avoid value function (6D) with obstacle-aware B_h feedback (Eq. 39)
"""
...
from reach_avoid_game.solvers.horizontal_solver import (
    ...
    solve_horizontal_max_distance_6d,
...
def main():
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix=f".horizontal_{args.preset}_", dir=temp_root))

    # Step 1: V_h_T (4D relative)
    t0 = time.time()
    v_h_t_path = solve_horizontal_max_distance(config, preset=args.preset, output_dir=temp_dir)
    t1 = time.time()
    print(f"  V_h_T computed in {t1 - t0:.1f}s")
    print()

    # Step 2: Attacker reaching (2D) — independent, can be in any order
    phi_a_path = solve_attacker_reaching(config, preset=args.preset, output_dir=temp_dir)
    t2 = time.time()
    print(f"  phi_A_reach computed in {t2 - t1:.1f}s")
    print()

    # Free JAX/numpy memory from previous solves before the large 6D solve
    gc.collect()

    # Step 3: obstacle-aware V_h_T for paper Algorithm 2 / Eq. 39
    gc.collect()
    v_h_6d_path = solve_horizontal_max_distance_6d(config, preset=args.preset, output_dir=temp_dir)
    t3 = time.time()
    print(f"  V_h_T_6d computed in {t3 - t2:.1f}s")
    print()

    # Step 4: B_h from obstacle-aware V_h_T_6d
    b_h_path = compute_invariant_set_Bh(v_h_6d_path, d_h=config.capture.d_h, output_dir=temp_dir)
    t4 = time.time()
    print(f"  B_h computed in {t4 - t3:.1f}s")
    print()

    # Step 5: Phi_h with obstacle-aware B_h feedback (Paper Eq. 39)
    v_h_t_data = load_value_function(v_h_6d_path)
    phi_h_path = solve_horizontal_reach_avoid(
        config, preset=args.preset, output_dir=temp_dir,
        v_h_t_data=v_h_t_data,
    )
    t5 = time.time()
    print(f"  Phi_h computed in {t5 - t4:.1f}s")
    print()

    for name in ["V_h_T.npz", "phi_A_reach.npz", "V_h_T_6d.npz", "B_h.npz", "phi_h.npz"]:
        shutil.move(str(temp_dir / name), str(final_output / name))
    shutil.rmtree(temp_dir, ignore_errors=True)

    print(f"Total computation time: {t5 - t0:.1f}s")
    print("All horizontal value functions computed and promoted successfully.")
